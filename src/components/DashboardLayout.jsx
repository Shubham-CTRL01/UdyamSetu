...
import {
  Rocket, ShieldCheck, ShieldAlert, Clock, LogOut, LogIn,
  Menu, Globe, Mail, Settings, Landmark, ChevronDown, ArrowRight
} from "lucide-react";

/* ── Flowchart sub-item ─────────────────────────────────────────── */
function SubNavItem({ icon: Icon, label, active, onClick, badge, isLast }) {
  return (
    <div className="flex items-stretch">
      {/* Vertical connector */}
      <div className="flex flex-col items-center shrink-0" style={{ width: 20 }}>
        <div className="w-px bg-slate-200" style={{ height: isLast ? 14 : "100%", minHeight: 14 }} />
        {!isLast && <div className="w-px bg-slate-200 flex-1" />}
      </div>
      {/* Horizontal tick + button */}
      <div className="flex items-center mb-1 flex-1 min-w-0">
        <div className="w-3 h-px bg-slate-200 shrink-0 mr-1" />
        <button
          onClick={onClick}
          type="button"
          className={`flex-1 flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all group ${
            active
        ...
        >
          ...
          {active && (
            <ArrowRight className="w-2.5 h-2.5 text-indigo-500 shrink-0 opacity-70" />
          )}
        </button>
      </div>
    </div>
...
/* ── Root nav item (Dashboard button) ─────────────────────────────── */
function RootNavItem({ icon: Icon, label, active, onClick, subItems, activeSection, onSectionChange }) {
  const hasSubs = subItems && subItems.length > 0;

  return (
    <div>
      {/* Dashboard button — root node */}
      <button
        onClick={onClick}
        type="button"
      ...
      >
        ...
        {hasSubs && (
          <ChevronDown
            className={`w-3.5 h-3.5 shrink-0 ${active ? "text-amber-300" : "text-slate-400"}`}
          />
        )}
      </button>

      {/* Flowchart tree — always visible */}
      {hasSubs && (
        <div className="mt-1 ml-3 pl-0">
          {/* "Sections" label with connecting line from dashboard root */}
          <div className="flex items-center ml-2 mb-0.5">
            <div className="w-px h-2 bg-slate-200 mr-3 ml-1.5" />
...
function SidebarContent({ profile, user, role, verificationStatus, navItems, activeSection, onSectionChange, onSignOut }) {
  ...
  // Split nav: dashboard root vs. all other items (sub-items in flowchart)
  const dashItem  = navItems.find((n) => n.id === "dashboard");
  const subItems  = navItems.filter((n) => n.id !== "dashboard");
    ...
    <aside className="flex flex-col h-full bg-white border-r border-slate-200 w-64 overflow-y-auto">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1">
        ...
        {dashItem && (
          <RootNavItem
            icon={dashItem.icon}
            label={dashItem.label}
            active={activeSection === dashItem.id}
            onClick={() => onSectionChange(dashItem.id)}
            subItems={subItems}
      ...
      {/* Sign out */}
      <div className="px-3 py-3 border-t border-slate-100 space-y-1.5 mt-auto shrink-0">
        {user ? (
          <>
            <Link
              to="/login"
