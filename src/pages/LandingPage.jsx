...
const ECOSYSTEMS = [
  {
    ...
    features: [
      "DPIIT Recognition & Registry sync",
      "Grand Challenge applications",
      "GeM vendor onboarding pipeline",
      "Section 80-IAC tax exemption tracking",
      "Investor matchmaking network",
      "Pilot deployment tracking dashboard",
    ],
  },
  {
    track: "Government & PSU",
    icon: Building2,
    color: "from-indigo-600 to-blue-700",
    features: [
      "JanParichay / MeriPehchaan SSO",
      "Ministry problem statement portal",
      "DPIIT-verified startup discovery",
      "Compliance & procurement audit logs",
      "GeM integration & tendering",
      "Real-time pilot monitoring console",
    ],
  },
];
...
function StatCard({ icon: Icon, value, label, color }) {
  return (
    <div className="flex flex-col items-center gap-1.5 p-5 bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow text-center">
      ...
      <div
        className="font-extrabold text-2xl text-slate-900 tracking-tight"
...
function ChallengeCard({ challenge, onNavigate }) {
  return (
    <div className="flex flex-col bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 overflow-hidden group">
      <div className="p-5 flex-1">
        <div className="flex items-start justify-between gap-3 mb-3">
          <span className={`text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full ${challenge.badgeColor}`}>
            {challenge.badge}
          </span>
        ...
        <h3 className="font-bold text-slate-900 text-sm leading-snug mb-3" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
          {challenge.title}
        </h3>
      ...
      <div className="px-5 py-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
        <div>
          <div className="text-[11px] text-slate-500">Grant Value</div>
          <div className="font-bold text-slate-900 text-sm">{challenge.grant}</div>
        </div>
        <div className="text-right mr-3">
          <div className="text-[11px] text-slate-500">Applicants</div>
          <div className="font-bold text-slate-900 text-sm">{challenge.applicants}</div>
        </div>
        <button
          onClick={() => onNavigate("/login")}
          className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold rounded-lg bg-[#0B192C] text-white hover:bg-[#1E3E62] transition-colors"
        >
    ...
    <div className="min-h-screen bg-white">
          ...
          <div className="max-w-3xl">
            ...
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/15 px-3.5 py-1.5 rounded-full mb-6">
              ...
              <span className="text-[11px] text-slate-200 tracking-widest font-semibold uppercase">
                Official Sovereign Digital Exchange Platform
              </span>
            </div>

            {/* Headline */}
            <h1
              className="font-extrabold text-4xl sm:text-5xl lg:text-[56px] tracking-tight leading-[1.1] mb-6"
              style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
            >
              India's National{" "}
              <span className="text-amber-400">GovTech &amp;</span>
              <br />
              <span className="text-amber-400">MSME</span> Innovation Bridge
            </h1>

            <p className="text-slate-300 text-base sm:text-lg leading-relaxed mb-8 max-w-2xl">
              A unified, interoperable exchange enabling public sector departments to pilot, procure, and scale verified indigenous innovations from startups &amp; MSMEs with zero friction.
            </p>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button
                onClick={() => onNavigate("/login")}
                className="flex items-center gap-2 px-6 py-3 bg-amber-400 hover:bg-amber-500 text-[#0B192C] font-bold text-sm rounded-xl transition-colors shadow-lg"
              >
              ...
              <button
                onClick={() => onNavigate("/login")}
                className="flex items-center gap-2 px-6 py-3 bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 text-white font-semibold text-sm rounded-xl transition-colors"
              >
                <Building2 className="w-4 h-4" />
                I'm from a Ministry / PSU
              </button>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-20">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-indigo-600 mb-3">
            <Zap className="w-3.5 h-3.5" /> Two-Sided National Platform
          </div>
          <h2
            className="font-extrabold text-3xl sm:text-4xl text-slate-900 tracking-tight"
            style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
          >
            Built for Every Stakeholder
          </h2>
          <p className="text-slate-500 text-base mt-3 max-w-xl mx-auto">
            Whether you're scaling your startup or running a national ministry, UdyamSetu has a dedicated portal for you.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {ECOSYSTEMS.map((eco) => (
            <div
              key={eco.track}
              className="rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-lg transition-shadow"
            >
              <div className={`h-2 bg-gradient-to-r ${eco.color}`} />
              <div className="p-6">
                <div className="flex items-center gap-3 mb-5">
                  <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${eco.color} flex items-center justify-center`}>
                ...
                <button
                  onClick={() => onNavigate("/login")}
                  className="w-full flex items-center justify-center gap-2 py-2.5 px-4 text-sm font-semibold rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100 text-slate-800 transition-colors"
                >
      ...
      <section className="bg-slate-50 py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
            <div>
              <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-amber-600 mb-2">
                <TrendingUp className="w-3.5 h-3.5" /> Featured National Challenges
              </div>
              <h2
                className="font-extrabold text-3xl sm:text-4xl text-slate-900 tracking-tight"
                style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
              >
                Live Grand Challenges
              </h2>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {CHALLENGES.map((c) => (
              <ChallengeCard key={c.id} challenge={c} onNavigate={onNavigate} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-emerald-600 mb-3">
            <Globe className="w-3.5 h-3.5" /> Certified Integrations
          </div>
          <h2
            className="font-extrabold text-3xl text-slate-900 tracking-tight"
            style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
          >
            Trusted by India's Digital Infrastructure
          </h2>
          <p className="text-slate-500 text-sm mt-2 max-w-lg mx-auto">
            UdyamSetu is deeply integrated with official national government systems and digital public infrastructure.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {INTEGRATIONS.map((intg) => (
            <div
              key={intg.name}
              className="flex flex-col items-center text-center p-4 bg-white border border-slate-200 rounded-2xl hover:shadow-md hover:-translate-y-0.5 transition-all duration-200"
            >
              <div className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center mb-2">
                <ShieldCheck className="w-5 h-5 text-slate-600" />
              </div>
              <div className="font-bold text-xs text-slate-900 mb-0.5">{intg.name}</div>
              <div className="text-[10px] text-slate-500">{intg.desc}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-gradient-to-br from-[#0B192C] via-[#10243E] to-[#1E3E62] py-16 px-4 sm:px-6 lg:px-8 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <h2
            className="font-extrabold text-3xl sm:text-4xl text-white tracking-tight mb-4"
            style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
          >
            Ready to bridge the gap?
          </h2>
          <p className="text-slate-400 text-base mb-8">
            Join 14,200+ startups, MSMEs, and 86 government bodies already on India's premier innovation exchange.
          </p>
